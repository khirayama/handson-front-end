...
現在（2015.09.01）、BabelなどのES2015を取り入れた環境が広まっていますが、ここではRuby on Railsの標準に乗っ取って進めます。  
...
以上の手順です。これはiOSやAndroidでもあまり違いはありません。BEGINNERの内容を例に見てみましょう。  
...
7. 見た目を整える  
6が完了したら、最後に見た目を整える。これは分業を行いやすくするとともに、JavaScript(CoffeeScript)が行うべき役割を切り分けれていることを確認する作業でもある
...
- もしthisを持ち回すときは```_this = this```を使用してください。一般的に```self```などもありますが、ここで重要なのは統一することです。
- eventオブジェクトの名前は、eやevtと略さずeventとしてください。```(event) => some.action()```
